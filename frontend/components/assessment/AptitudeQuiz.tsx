...
import { AlertCircle, Brain, ChevronLeft, ChevronRight, Loader2 } from "lucide-react";
...
export default function AptitudeQuiz() {
  ...
  useEffect(() => {
    // Try preloaded session first, fall back to fresh generation
    preloadAptitudeQuestions()
      .then((s) => {
        setSession(s);
  ...
  }, [session, timeLeft, answers, finish]);
  ...
  if (loading) {
    return (
      <div className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-[#111827] px-6">
        ...
        <p className="relative mt-5 font-medium text-white">Loading aptitude questions&hellip;</p>
        <p className="relative mt-1 text-sm text-slate-500">Should be instant if preloaded</p>
      </div>
    );
  }
      ...
      <div className="relative mx-auto max-w-3xl px-4 py-6">
        ...
        <div className="mt-5 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {submitting ? "Submitting\u2026" : isLast ? "Finish Aptitude" : "Next"}
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-2xl border border-white/10 bg-[#1E2640] p-7 shadow-2xl">
            ...
            <h3 className="mb-2 text-lg font-semibold text-white">Submit aptitude quiz?</h3>
